...
class TranslateURLHandler(webapp.RequestHandler):
    ...
    def post(self):
        self.requesthandler()
    def requesthandler(self):
        blocked_tags = ['script','style','embed', 'img', '<!--','!--']
        ...
        if len(url) > 0:
            ...
            for bt in blocked_tags:
                tags = soup.findAll(bt)
                for t in tags:
                    t.extract()
            tags = soup.findAll(text = True, recursive = True)
            for t in tags:
                if t is not None:
                    words = len(string.split(t.string,' '))
            ...
            <tr><td>[count] Count words before translating (y/n)</td><td><input type=text name=count value=y></td></tr>
            <tr><td colspan=2><input type=submit value=Submit></td></tr></form></table>
            """
            path = os.path.join(os.path.dirname(__file__), "doc.html")
            args = dict(
...
application = webapp.WSGIApplication([('/realtime/add', AddAgent),
                                    ...
                                    ('/translate/url', TranslateURLHandler),
